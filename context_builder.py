...
本文件为 MoFox_Bot 项目新增功能。

主要功能:
- 为 PFC 提供完整的情境感知能力
- 集成工具调用功能（类似 KFC 的 context_builder）
- 支持关系信息、记忆块、工具调用、表达习惯等上下文模块
...
class PFCContextBuilder:
    ...
    为提示词提供完整的情境感知数据，类似于 KFC 的 KFCContextBuilder。
    """
    ...
    def __init__(self, stream_id: str, pfc_config: "PFCConfig"):
        ...
        self.platform = "qq"  # PFC 主要用于 QQ 私聊
        self.is_group_chat = False  # PFC 专为私聊设计
    ...
    async def build_all_context(
        ...
        Returns:
            dict: 包含所有上下文块的字典
        """
        logger.debug(f"[PFC上下文] 开始构建上下文: sender={sender_name}, target={target_message[:50] if target_message else '(空)'}...")
        ...
        try:
            ...
            for result in task_results:
                if isinstance(result, tuple) and len(result) == 3:
                    ...
                    if duration > 8:
                        logger.warning(f"PFC 上下文构建耗时过长: {chinese_name} 耗时: {duration:.1f}s")
    ...
    async def _build_relation_info(self, sender_name: str, target_message: str, user_id: str | None = None) -> str:
        ...
        try:
            ...
            relationship_fetcher = relationship_fetcher_manager.get_fetcher(self.stream_id)

            user_relation_info = await relationship_fetcher.build_relation_info(person_id, points_num=5)
            stream_impression = await relationship_fetcher.build_chat_stream_impression(self.stream_id)
    ...
    async def _build_memory_block(
        ...
        try:
            ...
            if not unified_manager:
                logger.warning("[PFC记忆] 管理器初始化失败或未启用，跳过记忆检索")
                return ""

            # 构建查询文本
            query_text = target_message if target_message else chat_history[:500]
            logger.debug(f"[PFC记忆] 开始检索，查询文本: {query_text[:100]}...")
    ...
    async def _build_tool_info(
        ...
        try:
            ...
            tool_executor = ToolExecutor(chat_id=self.stream_id)
            ...
            try:
                ...
                if recalled_caches:
                    ...
                    info_parts.append("\n\n".join(recall_parts))
                    logger.info(f"[PFC缓存召回] 召回了 {len(recalled_caches)} 条相关搜索缓存")
            except Exception as e:
                logger.debug(f"[PFC缓存召回] 召回失败（非关键）: {e}")
            ...
            if tool_results:
                ...
                logger.info(f"[PFC工具调用] 获取到 {len(tool_results)} 个工具结果")

            # 如果没有任何信息，返回空字符串
            if not info_parts:
                logger.debug("[PFC工具调用] 未获取到任何工具结果或历史记录")
                return ""
...
async def build_pfc_context(
    ...
    """
    builder = PFCContextBuilder(stream_id, pfc_config)
    return await builder.build_all_context(sender_name, target_message, chat_history, user_id, enable_tool)
